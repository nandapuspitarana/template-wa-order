...
//require  get_template_directory() . '/inc/customizer-custom-controls/functions.php';
            ...
            <div class="formWaBody" id="formWABody">
                ...
                <form class="form" method="post" enctype="multipart/form-data" onsubmit="orderWA(this); return false;" action="<?php echo $ajax_url; ?>" style="display:none">
                    ...
                    <?php if (get_option('waorder_feature_payment_enable') == 'yes' ) : ?>
                        <?php
                        $provider = get_option('waorder_payment_provider', 'manual');
                        if( $provider == 'manual' ){
                            $manuals = get_option('waorder_payment_manual_lists');
                            $lists = array();
                            foreach( (array)$manuals as $m ){
                                $lists[$m['bank'].' '.$m['name']] = $m['bank'].' '.$m['name'];
                            }
                        }else{
                            $lists = apply_filters('waorder_form_payment_lists', $provider);
                        }
