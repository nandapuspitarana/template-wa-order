                                ...
                                <div class="order-item-box">
                                    ...
                                    <div class="order-item-detail">
                                        ...
                                        <?php if( isset($item['order_item_custom_name']) && isset($item['order_item_custom_value']) ): ?>
                                            <p><?php echo $item['order_item_custom_name']; ?>: <?php echo $item['order_item_custom_value']; ?></p>
                                        <?php endif; ?>
...
function waorder_order_save_metabox_view($post){
    ...
    <div class="waorder">
        <div class="order-customer waorder-clearfix">
            ...
            <p>
                <?php
                $print_link = str_replace('edit', 'print', get_edit_post_link( $post->ID ));
                ?>
                <a href="<?php echo $print_link; ?>" target="__blank">
                    <button type="button" style="width: 100%;" class="button button-hero">Cetak Invoice</button>
                </a>
            </p>
        </div>
    </div>
...
function waorder_order_content_column( $column, $post_id ) {
    ...
    switch ( $column ) :
        ...
        echo '<div style="text-align:right">';
            echo '<a href="'.get_edit_post_link( $post_id ).'" class="button">View Order</a>';
            $print_link = str_replace('edit', 'print', get_edit_post_link( $post_id ));
            echo '<p><a target="blank" href="'.$print_link.'" class="button">Cetak Invoice</a><p>';
            echo '</div>';
            break;
...
add_action('admin_init', 'waorder_order_printout');
function waorder_order_printout(){
    ...
    if( $_GET['action'] != 'print' ) return;

    $post = get_post(intval($_GET['post']));

    if( $post && $post->post_type != 'order' ) return;

    include( WAORDER_PATH . '/template/order-print.php');
    exit;
}
