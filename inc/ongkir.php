...
add_action('waorder_feature_option_page_ongkir_flatongkir', 'waorder_ongkir_options_page_flatongkir' );
function waorder_ongkir_options_page_flatongkir(){

    $lists = get_option('waorder_flatongkir_lists');

    ob_start();
    ?>
    <div class="waorder-flatongkir">
            ...
            <tbody>
                <?php foreach( (array)$lists as $key=>$val ): ?>
                    ...
                    <tr class="waorder-shortcoe-field wpapgfield">
                        <td><input type="text" name="waorder_flatongkir_lists[<?php echo $key; ?>][location]" value="<?php echo $val['location']; ?>"></td>
                        <td><input type="text" name="waorder_flatongkir_lists[<?php echo $key; ?>][cost]" value="<?php echo $val['cost']; ?>"></td>
                        <td>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                ...
                jQuery(".add-more").click(function(){
                    ...
                    html += '<td><input type="text" name="waorder_flatongkir_lists['+field_number+'][location]" placeholder="Lokasi"></td>';
                    html += '<td><input type="text" name="waorder_flatongkir_lists['+field_number+'][cost]" placeholder="Harga"></td>';
                    html += '<td>';
...
add_action('waorder_wa_form_before_total', 'waorder_wa_form_flatongkir_field' );
function waorder_wa_form_flatongkir_field(){

    if( get_option('waorder_feature_ongkir_enable') !== 'yes' ) return;

    if( get_option('waorder_ongkir_provider') !== 'flatongkir' ) return;

    $lists = get_option('waorder_flatongkir_lists', array());
                    ...
                    <div class="input">
                        ...
                        <select id="orderOngkir" name="order_ongkir" required oninvalid="this.setCustomValidity('Tentukan Ongkir')" oninput="this.setCustomValidity('')" onchange="chooseOngkir(this);">
                            ...
                            foreach( (array)$lists as $key=>$val ){
                                ?>
                                <option hidden="hidden" selected="selected" value="">Pilih Kurir</option>
                                <option value="<?php echo intval($val['cost']); ?>"><?php echo $val['location'].' - Rp '.number_format( intval($val['cost']),0,'.','.'); ?></option>
                                <?php
                            }
                            ?>
                        </select>
