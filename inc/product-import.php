...
function waorder_create_product($data)
{
    ...
    $d = array(
        ...
        'excerpt'               => isset($data[17]) ? sanitize_text_field($data[17]) : '',
    ...
    $post_data = array(
        ...
        'post_excerpt' => str_replace('*', ',', $d['excerpt']),
        'post_status'  => 'draft',
        //'post_author'  => 1,
        'post_type'    => 'product',
        // 'tax-input' => array(
...
function waorder_upload_product_thumbnail($url, $post_id)
{
    ...
    if ($url != '') {
        ...
        $file = array();
        $file['name'] = basename(parse_url($url, PHP_URL_PATH)) . '.jpg';
        $file['tmp_name'] = download_url($url);

        if (is_wp_error($file['tmp_name'])) {
            if (is_string($file['tmp_name'])) @unlink($file['tmp_name']);
            echo "Download Error: " . $file['tmp_name']->get_error_message() . "\n";
        } else {
            $attachmentId = media_handle_sideload($file, $post_id);

            if (is_wp_error($attachmentId)) {
                if (is_string($file['tmp_name'])) @unlink($file['tmp_name']);
                echo "Sideload Error: " . $attachmentId->get_error_message() . "\n";
            } else {
